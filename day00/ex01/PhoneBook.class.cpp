...
{
	std::string first_name_dup = this->first_name;
	std::string last_name_dup = this->last_name;
	std::string nickname_dup = this->nickname;


	if (first_name_dup.length() > 10)
	{
		first_name_dup.resize(9);
		first_name_dup.insert(9, 1, '.');
	}
	if (last_name_dup.length() > 10)
	{
		last_name_dup.resize(9);
		last_name_dup.insert(9, 1, '.');
	}
	if (nickname_dup.length() > 10)
	{
		nickname_dup.resize(9);
		nickname_dup.insert(9, 1, '.');
	}
	std::cout << std::setw(10) << id << '|';
	std::cout << std::setw(10) << first_name_dup << '|';
	std::cout << std::setw(10) << last_name_dup << '|';
	std::cout << std::setw(10) << nickname_dup << std::endl;
}
